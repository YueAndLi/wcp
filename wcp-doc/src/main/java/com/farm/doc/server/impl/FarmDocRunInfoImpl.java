...
import com.farm.doc.dao.*;
import com.farm.doc.domain.*;
...
public class FarmDocRunInfoImpl implements FarmDocRunInfoInter {
    ...
    public DataQuery createReleaseRankingSimpleQuery(DataQuery query) {
        DataQuery dbQuery = DataQuery.init(query,
                "(SELECT A.USERNAME AS USERNAME, A.DOCNUM AS DOCNUM, A.GOODNUM AS GOODNUM, A.BADNUM AS BADNUM,"
                        ...
                        + "	FROM (SELECT * FROM KNOW_FARM_DOC A WHERE A.STATE = '1') DOC "
                        + "	INNER JOIN KNOW_FARM_DOCRUNINFO DOCRUNINFO ON DOC.RUNINFOID = DOCRUNINFO.ID"
                        + "	LEFT JOIN KNOW_ALONE_AUTH_USER USER ON DOC.CUSER = USER.ID" + "	GROUP BY DOC.CUSER) A )B",
    ...
    public DataQuery getUserEnjoyDoc(String userId) {
        DataQuery query = DataQuery.getInstance("1",
                ...
                "KNOW_FARM_DOCENJOY a left join KNOW_FARM_DOC b on a.DOCID=b.ID and b.STATE = '1' left join KNOW_FARM_DOCRUNINFO c on c.id =b.RUNINFOID");
    ...
    public List<DocBrief> getNewKnowList(int pagesize) {
        DataQuery query = DataQuery.getInstance("1",
                ...
                "KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b ON a.RUNINFOID=b.ID LEFT JOIN KNOW_FARM_RF_DOCTYPE c ON c.DOCID=a.ID LEFT JOIN KNOW_FARM_DOCTYPE d ON d.ID=c.TYPEID   LEFT JOIN KNOW_ALONE_AUTH_USER e ON e.ID=a.CUSER");
        query.addRule(new DBRule("a.READPOP", "1", "="));
        query.addRule(new DBRule("a.STATE", "1", "="));
        query.addRule(new DBRule("d.READPOP", "0", "="));
        query.addRule(new DBRule("a.ISTOP", "0", "="));
    ...
    public List<DocBrief> getPubHotDoc(int num) {
        ...
        query.addRule(new DBRule("a.READPOP", "1", "="));
        query.addSort(new DBSort("b.VISITNUM", "desc"));
        query.addRule(new DBRule("d.READPOP", "0", "="));
        query.setNoCount();
        query.setCache(Integer.valueOf(FarmParameterService.getInstance().getParameter("config.wcp.cache.long")),
                CACHE_UNIT.second);
    ...
    public List<DocBrief> getTypeDocs(String typeid, String userid, int num) {
        ...
        query.addRule(new DBRule("d.TREECODE", doctype.getTreecode(), "like-"))
                ...
                .addSqlRule("and (a.READPOP='1' or a.CUSER='" + userid
                        + "' OR (a.READPOP = '2' AND 0 < (SELECT e.ID FROM KNOW_FARM_DOCGROUP_USER e WHERE e.PSTATE = 1 AND e.GROUPID = a.DOCGROUPID AND e.CUSER = '"
                        + userid + "')))");
        query.setPagesize(num);
        List<DocBrief> list = null;
        try {
    ...
    public List<DocBrief> getPubTopDoc(int num) {
        ...
        try {
            ...
            query.addSort(new DBSort("TOP.CTIME", "desc"));
            query.addRule(new DBRule("A.ISTOP", "1", "="));
            query = DataQuery.init(query, "KNOW_FARM_TOP TOP " + "LEFT JOIN KNOW_FARM_DOC A ON TOP.DOCID = A.ID and a.STATE='1' "
                            + "LEFT JOIN KNOW_FARM_RF_DOCTYPE B ON B.DOCID =A.ID " + "LEFT JOIN KNOW_FARM_DOCTYPE C ON C.ID=B.TYPEID "
                            + "LEFT JOIN KNOW_FARM_DOCGROUP D ON D.ID=A.DOCGROUPID "
                            + "LEFT JOIN KNOW_FARM_DOCRUNINFO DOCRUNINFO ON A.RUNINFOID = DOCRUNINFO.ID",
    ...
    public DataResult getStatGoodUsers(int i) {
        try {
            ...
            query.setPagesize(i);
            query = DataQuery.init(query,
                    "( SELECT d.ID AS ID,d.NAME AS NAME,SUM(b.PRAISEYES) AS SUMYES FROM KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b ON a.RUNINFOID = b.ID LEFT JOIN KNOW_ALONE_AUTH_USER d ON a.CUSER = d.ID where a.STATE='1' GROUP BY d.ID,d.NAME ) g",
                    "ID,NAME,SUMYES");
    ...
    public DataResult getStatGoodGroups(int i) {
        try {
            ...
            query.setPagesize(i);
            query = DataQuery.init(query,
                    "( SELECT d.ID   AS ID, d.GROUPNAME AS NAME,d.JOINCHECK as JOINCHECK, SUM(b.PRAISEYES) AS SUMYES FROM KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b ON a.RUNINFOID = b.ID LEFT JOIN KNOW_FARM_DOCGROUP d ON d.id=a.DOCGROUPID WHERE a.STATE='1' and d.ID IS NOT NULL GROUP BY d.ID,d.GROUPNAME,d.JOINCHECK) g",
    ...
    public DataResult getStatMostUsers(int i) {
        try {
            ...
            query.setPagesize(i);
            query = DataQuery.init(query,
                    "( SELECT b.ID AS ID, b.name AS NAME, COUNT(*) AS NUM FROM KNOW_FARM_DOC a LEFT JOIN KNOW_ALONE_AUTH_USER b ON a.CUSER = b.ID where a.STATE='1' GROUP BY b.id,b.NAME) g",
                    "ID,NAME,NUM");
    ...
    public DataResult getStatGoodDocs(int i) {
        try {
            ...
            query.setPagesize(i);
            query = DataQuery.init(query,
                    "KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b ON a.RUNINFOID=b.ID LEFT JOIN KNOW_FARM_DOCGROUP c ON a.DOCGROUPID=c.ID LEFT JOIN KNOW_ALONE_AUTH_USER d ON a.CUSER=d.ID",
    ...
    public Map<String, Object> getStatNumForDay() throws Exception {
        ...
        DataQuery query = new DataQuery();
        query = DataQuery.init(query,
                "(SELECT COUNT(id) AS num,LEFT(ctime,8) AS DATE FROM KNOW_FARM_DOC GROUP BY DATE) AS a", "NUM,DATE");
        ...
        DataQuery queryGood = DataQuery.init(query,
                "(SELECT COUNT(a.id) AS num,LEFT(a.ctime,8) AS DATE FROM KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b ON a.RUNINFOID=b.ID WHERE b.EVALUATE>0  GROUP BY DATE) AS a",
                "NUM,DATE");
    ...
    public DataResult getStatUser(User user) {
        try {
            ...
            sql.append(
                    "SELECT a.NAME AS NAME,a.ID AS ID,a.ctime AS ctime,(SELECT COUNT(*) FROM KNOW_FARM_DOC WHERE cuser=a.ID and STATE='1') num,(SELECT MIN(ctime) FROM KNOW_FARM_DOC WHERE cuser=a.ID) stime,(SELECT MAX(ctime) FROM KNOW_FARM_DOC WHERE cuser=a.ID) etime,");
            sql.append(
                    "(SELECT SUM(pb.PRAISEYES) FROM KNOW_FARM_DOC pa LEFT JOIN KNOW_FARM_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS OKNUM,");
            sql.append(
                    "(SELECT SUM(pb.PRAISENO) FROM KNOW_FARM_DOC pa LEFT JOIN KNOW_FARM_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS NONUM,");
            sql.append(
                    "(SELECT SUM(pb.EVALUATE) FROM KNOW_FARM_DOC pa LEFT JOIN KNOW_FARM_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS EVANUM,");
            sql.append(
                    "(SELECT SUM(pb.VISITNUM) FROM KNOW_FARM_DOC pa LEFT JOIN KNOW_FARM_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID and pa.STATE='1') AS VINUM,");
            sql.append(
                    "(SELECT COUNT(*) FROM KNOW_FARM_DOC pa LEFT JOIN KNOW_FARM_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID AND pb.EVALUATE>0 and pa.STATE='1') AS GOODNUM,");
            sql.append(
                    "(SELECT COUNT(*) FROM KNOW_FARM_DOC pa LEFT JOIN KNOW_FARM_DOCRUNINFO pb ON pb.id=pa.RUNINFOID WHERE cuser=a.ID AND pb.EVALUATE<0 and pa.STATE='1') AS BADNUM");
            sql.append(" FROM KNOW_ALONE_AUTH_USER a WHERE a.ID='" + user.getId() + "'");
    ...
    public DataResult getMyAuditingByUser(String userid, int pagesize, int pagenum) {
        DataQuery query = DataQuery.getInstance(pagenum,
                ...
                "KNOW_FARM_DOC_AUDIT b left join KNOW_FARM_DOC a on a.id=b.DOCID LEFT JOIN  KNOW_FARM_RF_DOCTYPE c ON c.DOCID=a.ID LEFT JOIN KNOW_FARM_DOCTYPE d ON d.ID=c.TYPEID  left join KNOW_FARM_DOCTEXT f on f.id=b.TEXTID");
