...
public class FarmtopServiceImpl implements FarmtopServiceInter {
    ...
    public DataQuery createFarmtopSimpleQuery(DataQuery query) {
        DataQuery dbQuery = DataQuery
                .init(query,
                        "KNOW_FARM_TOP TOP "
                                + "LEFT JOIN KNOW_FARM_DOC A ON TOP.DOCID = A.ID and a.STATE='1'"
                                + "LEFT JOIN KNOW_FARM_RF_DOCTYPE B ON B.DOCID =A.ID "
                                + "LEFT JOIN KNOW_FARM_DOCTYPE C ON C.ID=B.TYPEID "
    ...
    public DataQuery docTopChooseDocList(DataQuery query) {
        ...
        dbQuery.addSqlRule(" AND NOT EXISTS (SELECT 1 FROM KNOW_FARM_TOP TOP_ WHERE A.ID = TOP_.DOCID) ");
        return dbQuery;
