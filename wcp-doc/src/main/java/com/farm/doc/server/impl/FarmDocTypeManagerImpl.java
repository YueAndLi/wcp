...
public class FarmDocTypeManagerImpl implements FarmDocTypeInter {
    ...
    public List<TypeBrief> getTypeInfos(LoginUser user, String parentId) {
        ...
        query = DataQuery.init(query,
                "(SELECT a.NAME as NAME,a.readpop as READPOP,a.WRITEPOP as WRITEPOP, a.ID as ID, a.PARENTID as PARENTID, (SELECT COUNT(B1.ID) FROM KNOW_FARM_DOC B1 LEFT JOIN KNOW_FARM_RF_DOCTYPE B2 ON B1.ID = B2.DOCID LEFT JOIN KNOW_FARM_DOCTYPE B3 ON B3.ID = B2.TYPEID WHERE B1.STATE='1' and B3.TREECODE  LIKE CONCAT(A.TREECODE,'%') AND B1.STATE='1') AS NUM FROM KNOW_FARM_DOCTYPE AS a LEFT JOIN KNOW_FARM_DOCTYPE AS b ON b.ID = a.PARENTID WHERE 1 = 1 AND (a.TYPE = '1' OR a.TYPE = '3') AND a.PSTATE = '1' and (a.PARENTID='"
    ...
    public DataResult getTypeDocs(LoginUser user, String typeid, int pagesize, int currentPage) {
        ...
        query.addSqlRule("and (a.READPOP='1' or a.CUSER='" + userid
                + "' OR (a.READPOP = '2' AND 0 < (SELECT count(e.ID) FROM KNOW_FARM_DOCGROUP_USER e WHERE e.PSTATE = 1 AND e.GROUPID = a.DOCGROUPID AND e.CUSER = '"
                + userid + "')))");
    ...
    public List<TypeBrief> getPubTypes() {
        DataQuery query = DataQuery.init(new DataQuery(),
                "(SELECT a.NAME as NAME, a.ID as ID, a.PARENTID as PARENTID, (SELECT COUNT(B1.ID) FROM KNOW_FARM_DOC B1 LEFT JOIN KNOW_FARM_RF_DOCTYPE B2 ON B1.ID = B2.DOCID LEFT JOIN KNOW_FARM_DOCTYPE B3 ON B3.ID = B2.TYPEID WHERE  B1.STATE='1' and  B3.TREECODE  LIKE CONCAT(A.TREECODE,'%') AND B1.STATE='1') AS NUM FROM KNOW_FARM_DOCTYPE AS a WHERE 1 = 1 AND (TYPE = '1' OR TYPE = '3') AND PSTATE = '1' ORDER BY SORT ASC) AS e",
    ...
    public List<TypeBrief> getTypes() {
        DataQuery query = DataQuery.init(new DataQuery(), "KNOW_FARM_DOCTYPE",
    ...
    public List<TypeBrief> getPopTypesForReadDoc(LoginUser user) {
        DataQuery query = DataQuery.init(new DataQuery(),
                "(SELECT a.NAME as NAME,a.SORT as SORT, a.ID as ID, a.READPOP as READPOP,a.WRITEPOP AS WRITEPOP, a.AUDITPOP AS AUDITPOP, a.PARENTID AS PARENTID, (SELECT COUNT(B1.ID) FROM KNOW_FARM_DOC B1 LEFT JOIN KNOW_FARM_RF_DOCTYPE B2 ON B1.ID = B2.DOCID LEFT JOIN KNOW_FARM_DOCTYPE B3 ON B3.ID = B2.TYPEID WHERE  B1.STATE='1' and  B3.TREECODE  LIKE CONCAT(A.TREECODE,'%') AND B1.STATE='1') AS NUM,f.oid as OID,f.FUNTYPE as FUNTYPE FROM KNOW_FARM_DOCTYPE AS a left join KNOW_FARM_DOCTYPE_POP as f on f.TYPEID=a.ID WHERE 1 = 1 AND (TYPE = '1' OR TYPE = '3') AND PSTATE = '1' ) AS e",
    ...
    public List<TypeBrief> getTypesForWriteDoc(LoginUser user) {
        DataQuery query = DataQuery.init(new DataQuery(),
                "(SELECT a.NAME as NAME, a.ID as ID,a.TYPE as type, a.READPOP as READPOP,a.WRITEPOP AS WRITEPOP, a.AUDITPOP AS AUDITPOP, a.PARENTID AS PARENTID, (SELECT COUNT(B1.ID) FROM KNOW_FARM_DOC B1 LEFT JOIN KNOW_FARM_RF_DOCTYPE B2 ON B1.ID = B2.DOCID LEFT JOIN KNOW_FARM_DOCTYPE B3 ON B3.ID = B2.TYPEID WHERE B3.TREECODE  LIKE CONCAT(A.TREECODE,'%') AND B1.STATE='1') AS NUM,f.oid as OID,f.FUNTYPE as FUNTYPE FROM KNOW_FARM_DOCTYPE AS a left join KNOW_FARM_DOCTYPE_POP as f on f.TYPEID=a.ID WHERE   (TYPE = '1' OR TYPE = '3') AND PSTATE = '1' ORDER BY SORT ASC) AS e",
    ...
    public List<String> getUserReadTypeIds(LoginUser user) {
        ...
        // 获得所有用户分配到的分类权限（不包括公开的分类）
        {
            ...
            DataQuery query = DataQuery.getInstance("1", "TYPEID,ID", "KNOW_FARM_DOCTYPE_POP");
            query.setPagesize(1000);
            query.setNoCount();
    ...
    public List<String> getTypesForUserRead(LoginUser user) {
        ...
        DataQuery query = DataQuery.getInstance(1, "ID,NAME", "KNOW_FARM_DOCTYPE").setPagesize(1000);
