...
public class FarmDocManagerImpl implements FarmDocManagerInter {
    ...
    public DocEntire createDoc(DocEntire entity, LoginUser user) {
        try {
            ...
            {// 先保存text
                ...
                entity.getTexts().setPcontent("创建");
            ...
            {// 保存doc对象
                ...
                if (entity.getDoc().getDocdescribe() == null || entity.getDoc().getDocdescribe().trim().length() <= 0) {
                    if (entity.getTexts() != null && StringUtils.isNotEmpty(entity.getTexts().getText1())) {
                    ...
                    } else if (entity.getDoc() != null && entity.getDoc().getContent() != null) {
                        String describe = entity.getDoc().getContent();
                        if (StringUtils.isNotEmpty(describe)) {
                            entity.getDoc().setDocdescribe(describe.length() > 128 ? describe.substring(0, 128) : describe);
                        }
                    }
                }
                // 如果没有 tags就生成tags
                ...
                if (entity.getDoc().getIstop() != null && entity.getDoc().getIstop() == 1) {
                    createFarmtop(entity, user);
                }
            }
    ...
    public DocEntire editDoc(DocEntire paradoce, String editNote, LoginUser user) {
        ...
        if (paradoce.getDoc().getIstop() != null) {
            newDoce.getDoc().setIstop(paradoce.getDoc().getIstop());
            if (newDoce.getDoc().getIstop() == 0 && StringUtils.isNotEmpty(newDoce.getDoc().getFramtopid())) {
                newDoce.getDoc().setFramtopid("");
                Farmtop top = farmtopDaoImpl.getEntity(paradoce.getDoc().getFramtopid());
                if (top != null) {
                    farmtopDaoImpl.deleteEntity(top);
                }
            }
            if (newDoce.getDoc().getIstop() == 1 && StringUtils.isEmpty(newDoce.getDoc().getFramtopid())) {
                createFarmtop(newDoce, user);
            }
    ...
    public DataQuery createSimpleDocQuery(DataQuery query) {
        DataQuery dbQuery = DataQuery.init(query,
                "KNOW_FARM_DOC  A LEFT JOIN KNOW_FARM_RF_DOCTYPE B ON B.DOCID =A.ID LEFT JOIN KNOW_FARM_DOCTYPE C ON C.ID=B.TYPEID LEFT JOIN KNOW_FARM_DOCGROUP d ON d.ID=a.DOCGROUPID",
    ...
    public DataQuery createSimpleTypeQuery(DataQuery query) {
        DataQuery dbQuery = DataQuery.init(query, "KNOW_FARM_DOCTYPE a LEFT JOIN KNOW_FARM_DOCTYPE b ON a.PARENTID=b.id",
    ...
    public List<FarmDoctext> getDocVersions(String docId) {
        ...
        DataQuery dbQuery = DataQuery.getInstance("1", "ID,ETIME,CUSERNAME,DOCID,PCONTENT,PSTATE", "KNOW_FARM_DOCTEXT");
    ...
    @Override
    public Farmtop createFarmtop(DocEntire entity, LoginUser currentUser) {
        Farmtop top = new Farmtop();
        top.setCtime(TimeTool.getTimeDate14());
        top.setSort(1);
        top.setCuser(currentUser.getId());
        top.setCusername(currentUser.getName());
        top.setPstate("1");
        top.setDocid(entity.getDoc().getId());
        top = farmtopDaoImpl.insertEntity(top);
        entity.getDoc().setIstop(1);
        entity.getDoc().setFramtopid(top.getId());
        return top;
    }
