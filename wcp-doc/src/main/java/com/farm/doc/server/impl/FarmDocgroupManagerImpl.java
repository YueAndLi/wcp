...
public class FarmDocgroupManagerImpl implements FarmDocgroupManagerInter {
    ...
    public DataQuery createFarmDocgroupQueryJoinUser(DataQuery query) {
        DataQuery dbQuery = DataQuery.init(query, "KNOW_FARM_DOCGROUP a left join KNOW_FARM_DOCGROUP_USER b on b.GROUPID=a.id",
    ...
    public DataQuery createFarmDocgroupQuery(DataQuery query) {
        DataQuery dbQuery = DataQuery.init(query,
                "(select ID,(SELECT COUNT(a.id) FROM KNOW_FARM_DOC a WHERE a.DOCGROUPID=KNOW_FARM_DOCGROUP.ID ) as DOCNUM,PSTATE,PCONTENT,USERNUM,CUSERNAME,CTIME,GROUPNAME,GROUPNOTE,GROUPTAG,GROUPIMG,JOINCHECK from KNOW_FARM_DOCGROUP) b ",
    ...
    public DataQuery createFarmDocgroupUserSimpleQuery(DataQuery query) {
        DataQuery dbQuery = DataQuery.init(query,
                "KNOW_FARM_DOCGROUP_USER a left join KNOW_ALONE_AUTH_USER b on a.userid=b.id left join KNOW_FARM_DOCGROUP c on a.groupid=c.id",
    ...
    public List<User> getAllAdministratorByGroup(String groupId) {
        ...
        DataQuery query = DataQuery.getInstance(1,
                ...
                " KNOW_FARM_DOCGROUP a LEFT JOIN KNOW_FARM_DOCGROUP_USER b ON a.ID = b.GROUPID LEFT JOIN KNOW_ALONE_AUTH_USER c ON c.ID = b.USERID");
    ...
    public List<Map<String, Object>> getEditorGroupByUser(String userId) {
        DataQuery query = DataQuery.getInstance("1", "b.groupNAME AS NAME,b.ID AS id",
                "KNOW_FARM_DOCGROUP_USER a  LEFT JOIN KNOW_FARM_DOCGROUP b ON a.GROUPID=b.ID");
    ...
    public DataQuery getGroupNewDocQuery(DataQuery query, String groupId, LoginUser currentUser) {
        ...
        sqlform.append(
                " FROM KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b ON a.RUNINFOID = b.ID LEFT JOIN KNOW_FARM_RF_DOCTYPE c ON c.DOCID = a.ID LEFT JOIN KNOW_FARM_DOCTYPE d   ON d.ID = c.TYPEID LEFT JOIN KNOW_FARM_DOCGROUP_USER e   ON e.GROUPID=a.DOCGROUPID  ");
    ...
    public DataQuery createUserGroupDocQuery(DataQuery query, String userid) {
        ...
        sqlBuffer.append(
                " FROM KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b ON a.RUNINFOID = b.ID LEFT JOIN KNOW_FARM_RF_DOCTYPE c ON c.DOCID = a.ID");
        sqlBuffer.append(
                "  LEFT JOIN KNOW_FARM_DOCTYPE d ON d.ID = c.TYPEID LEFT JOIN KNOW_FARM_DOCGROUP_USER e ON e.GROUPID = a.DOCGROUPID LEFT JOIN KNOW_FARM_DOCGROUP f ON f.ID=e.GROUPID");
    ...
    public List<GroupBrief> getHotDocGroups(int num, LoginUser currentUser) {
        DataQuery query = DataQuery.init(new DataQuery(),
                "KNOW_FARM_DOCGROUP as a left join KNOW_FARM_DOCGROUP_USER as b on a.id=b.GROUPID and b.USERID='"
    ...
    public DataResult getNewGroupDoc(LoginUser user, String gourpid, int pagesize, Integer pagenum) {
        DataQuery query = DataQuery.init(new DataQuery(),
                "KNOW_FARM_DOC a LEFT JOIN KNOW_FARM_DOCRUNINFO b on a.RUNINFOID=b.ID left join KNOW_FARM_RF_DOCTYPE f on a.id=f.docid left join KNOW_FARM_DOCTYPE g on g.id=f.typeid  left join KNOW_FARM_DOCGROUP d on a.DOCGROUPID=d.ID left join KNOW_FARM_DOCGROUP_USER c on c.GROUPID=d.ID and c.USERID='"
                        + (user == null ? "none" : user.getId()) + "' LEFT JOIN KNOW_ALONE_AUTH_USER e ON e.ID = a.CUSER  ",
    ...
    public DataResult getGroupUser(String groupid, SearchType isApply, SearchType isAdmin, SearchType isEditor,
        ...
        DataQuery query = DataQuery.getInstance(page,
                ...
                "KNOW_FARM_DOCGROUP_USER a left join KNOW_ALONE_AUTH_USER b on a.userid=b.id");
        query.setPagesize(pagesize);
    ...
    public DataResult getGroupsByUser(String userid, int pagesize, Integer pagenum) {
        DataQuery query = DataQuery.init(new DataQuery(),
                "KNOW_FARM_DOCGROUP as a left join KNOW_FARM_DOCGROUP_USER as b on a.id=b.GROUPID",
                "a.ID AS ID, a.CTIME AS CTIME, a.ETIME AS ETIME, a.CUSERNAME AS CUSERNAME, a.CUSER AS CUSER, a.EUSERNAME AS EUSERNAME, a.EUSER AS EUSER, a.PSTATE AS PSTATE, a.PCONTENT AS PCONTENT, a.GROUPNAME AS GROUPNAME, a.GROUPNOTE AS GROUPNOTE, a.GROUPTAG AS GROUPTAG, a.GROUPIMG AS GROUPIMG, a.JOINCHECK AS JOINCHECK, a.USERNUM AS USERNUM, a.HOMEDOCID AS HOMEDOCID, b.USERID AS USERID");
        query.addSort(new DBSort("a.USERNUM", "desc"));
    ...
    public DataResult getGroups(int pagesize, Integer pagenum) {
        DataQuery query = DataQuery.init(new DataQuery(), "KNOW_FARM_DOCGROUP as a ",
