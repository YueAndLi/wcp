    ...
    <div class="col-md-12">
        <div style="color: #008000; font-size: 12px; font-weight: lighter;">
            ...
            <c:forEach var="node" items="${DOCE.currenttypes}" varStatus="status">
                /
                <a style="font-weight: bold" href="webtype/view${node.id}/Pub1.html" title="${node.name}">${node.name}</a>
            </c:forEach>
            <b>下,并于</b>
...
<div class="row">
    <div class="col-md-12" id="textsBoxId">
        <c:choose>
            <c:when test="${not empty DOCE.texts.text1}">
                <p>${DOCE.texts.text1}</p>
            </c:when>
            <c:otherwise>
                <p>${DOCE.doc.docdescribe}</p>
            </c:otherwise>
        </c:choose>
    </div>
</div>
