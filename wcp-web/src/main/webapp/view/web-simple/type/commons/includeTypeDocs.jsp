                ...
                <div class="media">
                    <a class="pull-right"
                      ...
                      <%-- href="webuser/PubHome.do?userid=${node.USERID }"--%>
                       style="max-width: 200px; text-align: center;text-decoration:none;cursor: default;margin-top: 30px;"
                       title="${node.TITLE }">
                        <%--<img class="img-rounded side_unit_author_img" alt="${node.USERNAME}"
                             src="<PF:basePath/>${node.PHOTOURL }">--%>
                        <br/>
                        <span style="font-weight: bold;font-size:14px;" class="side_unit_info">创建人：</span>
                        <br/> <span style="font-size: 14px;" class="side_unit_info">${node.USERNAME}</span>
                    </a>
                    <div class="media-body">
                        <div style="margin-left: 4px;" class="pull-left">
                            <div class="doc_node_title_box">
                                <a class="doc_node_title_min"
                                   href="webdoc/view/Pub${node.DOCID}.html">${node.TITLE}
                                    <c:if
                                            test="${node.DOMTYPE=='3'}">
                            ...
                            <div class="side_unit_info side_unit_info_left_box">
                                ...
                                阅读(${node.VISITNUM}) <%--评论(${node.ANSWERINGNUM})--%><PF:FormatTime date="${node.PUBTIME}" yyyyMMddHHmmss="yyyy年MM月dd日 HH:mm"/>
                            </div>
                        </div>
                    </div>
                </div>
...
<c:if test="${fun:length(docs.resultList) > 0}">
    <div class="row">
        <div class="col-xs-12">
            ...
            <div style="float:right;margin:24px 20px 24px 0;">
                显示第${(docs.currentPage -1)*10 +1}至
                <c:if test="${docs.currentPage*10 <= docs.totalSize }">
                    ${docs.currentPage*10}
                </c:if>
                <c:if test="${docs.currentPage*10 > docs.totalSize }">
                    ${docs.totalSize}
                </c:if>
                项,共${docs.totalSize}项
            </div>
        </div>
    </div>
</c:if>