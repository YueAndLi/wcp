...
public class DocLuceneController extends WebUtils {
    ...
    public ModelAndView show(Integer pagenum, HttpSession session, HttpServletRequest request) {
        ...
        if (topdocs == null) topdocs = new ArrayList<DocBrief>();
        if (topdocs.size() < 4) {
            // 最新知识
            List<DocBrief> newdocs = new ArrayList<DocBrief>();
            newdocs = farmDocRunInfoImpl.getNewKnowList(4);
            if (newdocs != null && newdocs.size() > 0) {
                topdocs.addAll(newdocs.subList(0, newdocs.size()<=(4 - topdocs.size())?newdocs.size():(4-topdocs.size())));
            }
        }
