...
public class DocTypeController extends WebUtils {
    ...
    public ModelAndView manage(HttpSession session) {
        ...
        if (isOpen.equals("true")) {
            return ViewMode.getInstance().returnModelAndView(ThemesUtil.getThemePath() + "/type/manage");
        } else {
            String msg = "没有权限!";
            return ViewMode.getInstance().putAttr("MESSAGE", msg).returnModelAndView(ThemesUtil.getThemePath() + "/error");
        }
    }
    ...
    public ModelAndView list(HttpSession session) {
        ...
        return ViewMode.getInstance().returnModelAndView(ThemesUtil.getThemePath() + "/type/DoctypeManage");
