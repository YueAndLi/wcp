    ...
    private List<Map<String, Object>> initRoleGruopTreeNode(DataQuery query, String ids) {
        ...
        try {
            query = DataQuery
                    .init(
                            query,
                            "KNOW_ALONE_AUTH_ACTIONTREE A LEFT JOIN KNOW_ALONE_AUTH_ACTION B ON A.ACTIONID = B.ID LEFT JOIN (SELECT POSTID, MENUID FROM KNOW_ALONE_AUTH_POSTACTION WHERE POSTID = '"
